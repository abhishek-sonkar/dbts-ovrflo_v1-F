...
function App() {
  ...
  const getUserName = async () => {
    return await fetch("https://dbts-ovrflo-v1-b.onrender.com/userinfo", {
      method: "GET",
      headers: {
  ...
  const signupHandler = (fname, lname, email, password) => {
    fetch("https://dbts-ovrflo-v1-b.onrender.com/signup", {
      method: "POST",
      ...
      .then((r) => {
        ...
        .then((r) => {
          if(r.success === true) {
            toast.success("SignedUp successfully, please login to continue", { 
              autoClose: 2500, 
              closeButton: false, 
              hideProgressBar: true 
            });
  ...
  const loginHandler = (email, password) => {
    fetch("https://dbts-ovrflo-v1-b.onrender.com/login", {
      method: "POST",
  ...
  const postQuestionHandler = (title, quesBody, tags,) => {
    ...
    if (title.length !== 0 && quesBody.length !== 0) {
      fetch("https://dbts-ovrflo-v1-b.onrender.com/post-question", {
        method: "POST",
  ...
  const solveQuestionHandler = (ansBody, reqUrl) => {
    ...
    if(ansBody.length !== 0) {
      fetch("https://dbts-ovrflo-v1-b.onrender.com/solve-question?" + reqUrl, {
        method: "POST",
  ...
  const votesHandler = (quesId, ansId, vote) => {
    fetch(`https://dbts-ovrflo-v1-b.onrender.com/update-votes?qid=${quesId}&aid=${ansId}&vote=${vote}`, {
      method: "POST",
      headers: {
          "Content-Type": "application/json",
          "x-access-token": localStorage.getItem("token"),
  ...
  const logoutHandler = () => {
    fetch("https://dbts-ovrflo-v1-b.onrender.com/logout")
    .then(r => {
      if(r.ok) {
        setLoggedIn(false);
